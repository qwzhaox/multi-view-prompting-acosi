...
def init_args():
    ...
    parser.add_argument(
        "--task",
        default="asqp",
        choices=["asqp", "acos", "acosi", "aste", "tasd", "unified", "unified3"],
        type=str,
    ...
    parser.add_argument(
        "--output_dir", default="outputs/temp", type=str, help="Output directory"
    )
    parser.add_argument(
        "--load_ckpt_name", default=None, type=str, help="load ckpt path"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--seed", type=int, default=25, help="random seed for initialization"
    )
    ...
    parser.add_argument(
        "--ctrl_token", default="post", choices=["post", "pre", "none"], type=str
    )
    parser.add_argument(
        "--sort_label", action="store_true", help="sort tuple by order of appearance"
    )
    parser.add_argument(
        "--load_path_cache", action="store_true", help="load decoded path from cache"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="vote",
        choices=["vote", "rand", "heuristic", "pre_rank", "post_rank"],
    )
    parser.add_argument(
        "--data_ratio", default=1.0, type=float, help="low resource data ratio"
    )
...
class T5FineTuner(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            "scheduler": get_linear_schedule_with_warmup(
                optimizer, **self.config.lr_scheduler_init
            ),
            "interval": "step",
        }
    ...
    def train_dataloader(self):
        ...
        dataloader = DataLoader(
            ...
            drop_last=True
            if args.data_ratio > 0.3
            else False,  # don't drop on few-shot
            shuffle=True,
            num_workers=2,
        )
    ...
    def val_dataloader(self):
        ...
        return DataLoader(
            val_dataset, batch_size=self.config.eval_batch_size, num_workers=2
        )
    ...
    def prefix_allowed_tokens_fn(
        self, task, data_name, source_ids, batch_id, input_ids
    ):
        """
        ...
        if not os.path.exists("./force_tokens.json"):
            ...
            for task in force_words.keys():
                ...
                for dataset in force_words[task].keys():
                    ...
                    for w in cur_list:
                        tokenize_res.extend(
                            self.tokenizer(w, return_tensors="pt")[
                                "input_ids"
                            ].tolist()[0]
                        )
            ...
            for k, v in cate_list.items():
                ...
                for w in v:
                    tokenize_res.extend(
                        self.tokenizer(w, return_tensors="pt")["input_ids"].tolist()[0]
                    )
                dic["cate_tokens"][k] = tokenize_res
            sp_tokenize_res = []
            for sp in ["great", "ok", "bad"]:
                sp_tokenize_res.extend(
                    self.tokenizer(sp, return_tensors="pt")["input_ids"].tolist()[0]
                )
            for task in force_words.keys():
            ...
            for ie in ["direct", "indirect"]:
                impl_expl_tokenize_res.extend(
                    self.tokenizer(ie, return_tensors="pt")["input_ids"].tolist()[0]
                )
            dic["impl_expl_tokens"] = impl_expl_tokenize_res
            special_tokens_tokenize_res = []
            for w in ["[O", "[A", "[S", "[C", "[I", "[SS"]:
                special_tokens_tokenize_res.extend(
                    self.tokenizer(w, return_tensors="pt")["input_ids"].tolist()[0]
                )
            special_tokens_tokenize_res = [
                r for r in special_tokens_tokenize_res if r != 784
            ]
        ...
        last_right_brace_pos = (
            right_brace_index[-1][0] if right_brace_index.nelement() > 0 else -1
        )
        last_left_brace_pos = (
            left_brace_index[-1][0] if left_brace_index.nelement() > 0 else -1
        )
        cur_id = input_ids[-1]
        ...
        if last_left_brace_pos == -1:
            ...
            last_left_brace_pos != -1 and last_right_brace_pos == -1
...
def evaluate(model, task, data, data_type):
    ...
    cache_file = os.path.join(
        args.output_dir,
        "result_{}{}{}_{}_path{}_beam{}.pickle".format(
    ...
    if args.load_path_cache:
        ...
        data_loader = DataLoader(
            dataset, batch_size=args.eval_batch_size, num_workers=2
        )
        ...
        for batch in tqdm(data_loader):
            ...
            outs = model.model.generate(
                ...
                prefix_allowed_tokens_fn=partial(
                    model.prefix_allowed_tokens_fn, task, data, batch["source_ids"]
                )
        ...
        for i in range(0, len(targets)):
            ...
            if args.agg_strategy == "post_rank":
                ...
                for s in multi_outputs:
                    all_quints.extend(extract_spans_para(seq=s, seq_type="pred"))
                ...
                for q in output_quints:
                    ...
                    if tasks[i] == "aste":
                        if "null" not in [
                            ...
                            ot,
                            sp,
                        ]:  # aste has no 'null', for zero-shot only
                        ...
                        output.append(f"[A] {at} [O] {ot} [S] {sp} [C] {ac} [I] {ie}")
                    else:
                        raise NotImplementedError

                target_quints = extract_spans_para(seq=targets[i], seq_type="gold")

                if sorted(target_quints) != sorted(output_quints):
                    print("task, data:", tasks[i], datas[i])
                    print("target:", sorted(target_quints))
                    print("output:", sorted(output))
                    print("sent:", sents[i])
                    print("counter:", counter)
                    print("output quints:", output)
                    print("multi_path:", multi_outputs)
                    print()

                # if no output, use the first path
                output_str = " [SSEP] ".join(output) if output else multi_outputs[0]
    ...
    scores, all_labels, all_preds = compute_scores(outputs, targets, verbose=True)
    return scores
...
def train_function(args):
    ...
    if args.do_train:
        ...
        for i in range(0, min(10, len(dataset))):
            ...
            print("Input :", tokenizer.convert_ids_to_tokens(data_sample["source_ids"]))
            print(
                "Output:",
        ...
        # config optimizer
        t_total = (
            (len(train_loader.dataset) // (args.train_batch_size * max(1, args.n_gpu)))
        ...
        early_stop_callback = EarlyStopping(
            monitor="val_f1", min_delta=0.00, patience=20, verbose=True, mode="max"
        )
            ...
            if args.multi_task:
                ...
                for task in task_data_list:
                    for data in task_data_list[task]:
                        scores = evaluate(
                            model, task, data, data_type=args.eval_data_split
                        )
                ...
                scores = evaluate(
                    model, args.task, args.dataset, data_type=args.eval_data_split
                )
